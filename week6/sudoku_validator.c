...
1. pthread_t *thread: 쓰레드가 성공적으로 생성되었을 때 생성된 쓰레드를 식별하기 위해 사용되는 쓰레드 식별자
2. const pthread_attr_t *attr: 쓰레드 특성을 지정, default로 지정할 경우 NULL로 설정
3. *(*start_routine)(void *): thread가 실행할 함수
...
void *is3x3Valid(void* param) {
	...
	for (i = row; i < row + 3; i++) {
		for (j = col; j < col + 3; j++) {
			int num = sudoku[i][j];
			if (num < 1 || num > 9 || validityArray[num - 1] == 1) { // 0 이하거나 10 이상이거나 이미 있는 값이면
				/* TODO: exit thread */
				pthread_exit((void*)&retval);
			} else {
				validityArray[num - 1] = 1;		
...
int main() {	
	...
	for (i = 0; i < 9; i++) {
		for (j = 0; j < 9; j++) {						
			...
			if (i%3 == 0 && j%3 == 0) { // (0,0), (0,3), (0,6), (3,0), (3,3), (3,6), (6,0), (6,3), (6,6)
				parameters *data = (parameters *) malloc(sizeof(parameters));	//이 쓰레드용 구조체 선언
				data->row = i;		
				data->column = j;
				/* TODO: create thread */
